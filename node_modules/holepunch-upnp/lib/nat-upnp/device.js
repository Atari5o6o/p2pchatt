...
Device.prototype.run = function run(action, args, callback) {
  ...
  this.getService(this.services, function(err, info) {
    ...
    var body = '' +
          ...
          '<s:Body>' +
            '<u:' + action +
              ' xmlns:u=' + JSON.stringify(info.service) + '>' +
    ...
    request({
      ...
      var parser = new xml2js.Parser();
      parser.parseString(bodyStr, function(parseerr, body) {
        if (parseerr) {
          parseerr.responseText = bodyStr;
          callback(parseerr);
          return;
        }

        var soapns = natUtils.getNamespace(
          body,
          'http://schemas.xmlsoap.org/soap/envelope/'
        );

        if (res.statusCode !== 200) {
          parseerr = new Error('Request failed: ' + res.statusCode + ' ' + body);
          parseerr.responseText = bodyStr;
          parseerr.body = body;
        }

        callback(parseerr, body[soapns + 'Body']);
      });
    });
  });
};
